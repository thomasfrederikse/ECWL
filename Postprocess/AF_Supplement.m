...
%% Save amplification factors for RCP scenarios for the SI
clear all; close all; clc;
addpath(genpath('~/Scripts/matlab/'));

GMT_2050 = '~/Scripts/GMT/Papers/global_extremes/Supplement/SI_AF_2050/';
GMT_2100 = '~/Scripts/GMT/Papers/global_extremes/Supplement/SI_AF_2100/';
...
for s=1:numel(scn)
    for y=1:numel(year)
        for a=1:numel(ais)
            ...
            fname = strcat(GMT_2050,'af_RCP',scn{s},'_',ais{a},'.txt');
...
for s=1:numel(scn)
    for y=1:numel(year)
        for a=1:numel(ais)
            ...
            fname = strcat(GMT_2100,'af_RCP',scn{s},'_',ais{a},'.txt');
            dlmwrite(fname,[loc_array,amp_fac], 'delimiter',' ','precision','%.3f');
        end
    end
end